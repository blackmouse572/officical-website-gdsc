...
function BecomeAMemberButton({ className, ...props }: Props) {
  return (
    <>
      <Button
        as={Link}
      ...
      <Icons.sparkle className="w-4 h-4 text-purple-500 absolute -top-4 right-3 fill-purple-500 animate-ping opacity-0 scale-0 group-hover:scale-100 delay-200 rotate-45 direction-reverse duration-700 ease-out group-hover:running group-hover:opacity-100" />
      ...
      <Icons.sparkle className="w-6 h-6 text-purple-500 absolute right-0 -top-2 fill-purple-500 duration-700 animate-ping opacity-0 scale-0 rotate-6 group-hover:scale-100  ease-out group-hover:running group-hover:opacity-100" />
    </>
  );
}
